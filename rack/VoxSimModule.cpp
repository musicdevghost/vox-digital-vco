...
void VoxSimModule::process(const ProcessArgs& args) {
    if (frameIndex_ >= kBlock) {
        ...
        hal_.knobSpread01 = params[PARAM_SPREAD].getValue();

        hal_.attPitch  = params[PARAM_ATT_PITCH].getValue();
        hal_.attMorph  = params[PARAM_ATT_MORPH].getValue();
        hal_.attTimbre = params[PARAM_ATT_TIMBRE].getValue();
        hal_.attSpread = params[PARAM_ATT_SPREAD].getValue();

        hal_.cvPitchV  = inputs[INPUT_CV_PITCH].getVoltageSum();
        hal_.cvMorphV  = inputs[INPUT_CV_MORPH].getVoltageSum();
        hal_.cvTimbreV = inputs[INPUT_CV_TIMBRE].getVoltageSum();
        hal_.cvSpreadV = inputs[INPUT_CV_SPREAD].getVoltageSum();
